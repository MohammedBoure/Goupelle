...
# ==========================================
# 1. نافذة ضبط الشعار المتقدمة (Logo Dialog)
# ==========================================
...
# ==========================================
# 2. النافذة الرئيسية لإعدادات الملصق
# ==========================================
class SettingsDialog(QDialog):
    ...
    def load_config(self):
        self.config = {
            ...
            "elements": {
                ...
                "purity": {"show": True, "text": "Titre: 751.54", "x": 20, "y": 8, "size": 12, "font": "arial.ttf", "angle": 0},
                "extra": {"show": True, "text": "mg/g"},
                "weight": {"show": True, "text": "Poids: 4.25 g", "x": 5, "y": 14, "size": 12, "font": "arial.ttf", "angle": 0},
                "date": {"show": False, "text": "17/04/2026", "x": 5, "y": 18, "size": 10, "font": "arial.ttf", "angle": 0}
            }
        }
    ...
    def init_ui(self):
        ...
        self.cmb_orient.addItems(["Horizontal", "Vertical"])
        self.cmb_orient.setCurrentText(self.config["orientation"])
        self.cmb_orient.currentTextChanged.connect(self.trigger_preview)
        ...
        self.sp_offset_x.valueChanged.connect(self.trigger_preview)
        self.sp_offset_y.valueChanged.connect(self.trigger_preview)

        f_gen.addRow("الطابعة:", self.cmb_printer)
        f_gen.addRow("الاتجاه العام للملصق:", self.cmb_orient)
        f_gen.addRow("العرض (mm):", self.sp_w)
        f_gen.addRow("الارتفاع (mm):", self.sp_h)
        f_gen.addRow("الفاصل بين الملصقات (Gap):", self.sp_gap)
        f_gen.addRow("سحب شامل أفقي (X mm):", self.sp_offset_x)
        f_gen.addRow("سحب شامل عمودي (Y mm):", self.sp_offset_y)
        ...
        self.chk_logo = QCheckBox("تفعيل الشعار")
        self.chk_logo.setChecked(lg.get("show", False))
        self.chk_logo.stateChanged.connect(self.trigger_preview)
        
        self.lbl_logo_path = QLabel(lg.get("path", "لا يوجد شعار"))
        self.lbl_logo_path.setStyleSheet("color: gray; font-size: 11px;")
        ...
        self.cmb_logo_angle = QComboBox()
        self.cmb_logo_angle.addItems(["0°", "90°", "180°", "270°"]); self.cmb_logo_angle.setCurrentText(f"{lg.get('angle', 0)}°")
        self.cmb_logo_angle.currentTextChanged.connect(self.trigger_preview)
        
        h_logo_pos.addWidget(QLabel("X:")); h_logo_pos.addWidget(self.sp_logo_x)
        h_logo_pos.addWidget(QLabel("Y:")); h_logo_pos.addWidget(self.sp_logo_y)
        h_logo_pos.addWidget(QLabel("زاوية:")); h_logo_pos.addWidget(self.cmb_logo_angle)
        
        l_logo.addWidget(self.chk_logo)
        l_logo.addWidget(self.lbl_logo_path)
        l_logo.addLayout(btn_h)
        l_logo.addLayout(h_logo_pos)
        form_layout.addWidget(grp_logo)

        # 3. العناصر والنصوص
        grp_el = QGroupBox("📝 3. تخصيص النصوص")
        ...
        def create_element_row(key, label):
            ...
            inp = QLineEdit(el.get("text", "")); inp.textChanged.connect(self.trigger_preview)
            ...
            cmb_f = QComboBox()
            cmb_f.addItems(["arial.ttf", "tahoma.ttf", "times.ttf", "cour.ttf", "calibri.ttf"])
            cmb_f.setCurrentText(el.get("font", "arial.ttf"))
            cmb_f.currentTextChanged.connect(self.trigger_preview)
            
            cmb_a = QComboBox()
            cmb_a.addItems(["0°", "90°", "180°", "270°"])
            cmb_a.setCurrentText(f"{el.get('angle', 0)}°")
            cmb_a.currentTextChanged.connect(self.trigger_preview)
            
            h_bot.addWidget(QLabel("X:")); h_bot.addWidget(sx)
            h_bot.addWidget(QLabel("Y:")); h_bot.addWidget(sy)
            h_bot.addWidget(QLabel("حجم:")); h_bot.addWidget(sz)
            h_bot.addWidget(cmb_f)
            h_bot.addWidget(cmb_a)
            
            row_main.addLayout(h_top); row_main.addLayout(h_bot)
            v_el.addLayout(row_main)
            
            setattr(self, f"chk_{key}", chk); setattr(self, f"inp_{key}", inp)
            setattr(self, f"sp_{key}_x", sx); setattr(self, f"sp_{key}_y", sy)
        ...
        self.chk_extra = QCheckBox("➕ إضافة لاحقة للعيار (مثال: mg/g):")
        self.chk_extra.setChecked(el_extra.get("show", True))
        self.chk_extra.setStyleSheet("color: #2980b9; font-weight: bold;")
        self.chk_extra.stateChanged.connect(self.trigger_preview)
        
        self.inp_extra = QLineEdit(el_extra.get("text", "mg/g"))
        self.inp_extra.textChanged.connect(self.trigger_preview)
        
        row_extra.addWidget(self.chk_extra)
        row_extra.addWidget(self.inp_extra)
        ...
        btn_save.setStyleSheet("background-color: #2980b9; color: white; padding: 12px; font-weight: bold;")
        btn_save.clicked.connect(self.save_config)
        btn_test = QPushButton("🖨️ طباعة اختبار")
        btn_test.setStyleSheet("background-color: #e67e22; color: white; padding: 12px; font-weight: bold;")
        btn_test.clicked.connect(self.print_test)
        ...
        h_header.addWidget(QLabel("<b>👁️ المعاينة المباشرة </b>"))
        h_header.addStretch()
        
        btn_zoom_out = QPushButton("🔍 -")
        btn_zoom_out.setFixedSize(40, 30)
        btn_zoom_out.clicked.connect(self.zoom_out)
        
        btn_zoom_reset = QPushButton("1:1")
        btn_zoom_reset.setFixedSize(40, 30)
        btn_zoom_reset.clicked.connect(self.zoom_reset)
        
        btn_zoom_in = QPushButton("🔍 +")
        btn_zoom_in.setFixedSize(40, 30)
        btn_zoom_in.clicked.connect(self.zoom_in)
        
        h_header.addWidget(btn_zoom_out)
        h_header.addWidget(btn_zoom_reset)
        h_header.addWidget(btn_zoom_in)
        
        preview_layout.addLayout(h_header)
        
        self.lbl_preview = QLabel()
        self.lbl_preview.setAlignment(Qt.AlignCenter)
        self.lbl_preview.setStyleSheet("background: white; border: 2px solid #34495e; border-radius: 4px;")
        p_scroll = QScrollArea()
        p_scroll.setAlignment(Qt.AlignCenter)
        p_scroll.setWidget(self.lbl_preview)
    ...
    def clear_logo(self):
        self.lbl_logo_path.setText("لا يوجد شعار")
        self.chk_logo.setChecked(False)
        self.trigger_preview()

    def open_logo_dialog(self):
        path = self.lbl_logo_path.text()
        if not os.path.exists(path):
            QMessageBox.warning(self, "تنبيه", "الرجاء اختيار صورة شعار صالحة أولاً.")
            return
        dlg = LogoSettingsDialog(path, self.logo_settings, self)
        if dlg.exec():
            self.logo_settings = dlg.get_final_settings()
            self.trigger_preview()
    ...
    def get_pil_image(self):
        DPI_MULT = 8 
        w_px = int(self.sp_w.value() * DPI_MULT)
        h_px = int(self.sp_h.value() * DPI_MULT)
        if w_px <= 0 or h_px <= 0: return None

        # حساب الإزاحة الشاملة (Global Offset)
        global_off_x = int(self.sp_offset_x.value() * DPI_MULT)
        global_off_y = int(self.sp_offset_y.value() * DPI_MULT)
        ...
        if self.chk_logo.isChecked() and os.path.exists(self.lbl_logo_path.text()):
            try:
                ...
                if new_w > 0 and new_h > 0:
                    ...
                    logo_l = logo.convert("L")
                    logo_l = logo_l.point(lambda p: 0 if p < thresh else 255, '1')
                    ...
                    # تطبيق الإزاحة الشاملة على الشعار
        ...
        for key in elements_keys:
            if getattr(self, f"chk_{key}").isChecked():
                ...
                self._draw_rotated_text(img, text, x, y, font, angle)

        if self.cmb_orient.currentText() == "Vertical":
    ...
    def generate_preview(self):
        ...
        self._preview_data = img_rgba.tobytes("raw", "RGBA") 
        ...
        scaled_w = int(img.width * self.zoom_factor)
        scaled_h = int(img.height * self.zoom_factor)
        scaled_pix = pix.scaled(scaled_w, scaled_h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        
        self.lbl_preview.setPixmap(scaled_pix)
        self.lbl_preview.setFixedSize(scaled_pix.size())

    def save_config(self):
        self.config["printer_name"] = self.cmb_printer.currentText()
        self.config["label_width_mm"] = self.sp_w.value()
        self.config["label_height_mm"] = self.sp_h.value()
        self.config["gap_mm"] = self.sp_gap.value()
        self.config["offset_x_mm"] = self.sp_offset_x.value()
        self.config["offset_y_mm"] = self.sp_offset_y.value()
        self.config["orientation"] = self.cmb_orient.currentText()
        
        self.config["logo"] = {
            "show": self.chk_logo.isChecked(),
            "path": self.lbl_logo_path.text() if self.lbl_logo_path.text() != "لا يوجد شعار" else "",
            "x": self.sp_logo_x.value(),
            "y": self.sp_logo_y.value(),
            "angle": int(self.cmb_logo_angle.currentText().replace("°", ""))
        }
        ...
        for key in elements_keys:
            self.config["elements"][key] = {
                "show": getattr(self, f"chk_{key}").isChecked(),
                "text": getattr(self, f"inp_{key}").text(),
                "x": getattr(self, f"sp_{key}_x").value(),
                "y": getattr(self, f"sp_{key}_y").value(),
                "size": getattr(self, f"sp_{key}_sz").value(),
                "font": getattr(self, f"cmb_{key}_f").currentText(),
                "angle": int(getattr(self, f"cmb_{key}_a").currentText().replace("°", ""))
            }

        self.config["elements"]["extra"] = {
            "show": self.chk_extra.isChecked(),
            "text": self.inp_extra.text()
        }

        try:
            with open(self.config_file, 'w', encoding='utf-8') as f:
                json.dump(self.config, f, indent=4, ensure_ascii=False)
            QMessageBox.information(self, "نجاح", "تم حفظ إعدادات القالب والخطوط بنجاح!")
        except Exception as e:
    ...
    def print_test(self):
        ...
        if not printer_name:
            QMessageBox.warning(self, "تنبيه", "الرجاء اختيار الطابعة من القائمة العلوية.")
            return

        img = self.get_pil_image()
        if not img: return

        w_mm = f"{self.sp_w.value():g}"
        h_mm = f"{self.sp_h.value():g}"
        gap_mm = f"{self.sp_gap.value():g}"
        ...
        cmds = [
            ...
            f"BITMAP 0,0,{w_bytes},{img.height},0,"
        ]
        ...
        try:
            ...
            QMessageBox.information(self, "نجاح", "تم إرسال أمر الطباعة إلى الطابعة.")
        except Exception as e:
            QMessageBox.critical(self, "خطأ بالطباعة", f"تعذر إرسال الأمر للطابعة.\n{str(e)}")